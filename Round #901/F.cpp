...
void solve_test_case() {
    ...
    vector<double> counted_deg_probs (n + 1);
    counted_deg_probs[0] = 0;
    counted_deg_probs[1] = 1.0;
    counted_deg_probs[2] = 0.5;
    for (int deg = 2; deg <= n; ++deg) {
        counted_deg_probs[deg] = 1.0 / (deg ) ;
        // counted_deg_probs[deg] = 1.0 / (deg ) + counted_deg_probs[deg - 2] * (1.0 - 1.0 / deg);
    }
    for (int start = n - 2; start >= 0; --start) {
        int deg = graph[start].size();
        if (deg == 0) {
            probability[start] = 0;
            continue;
        }
        for (int next: graph[start]) {
            probability[start] = max(probability[start], probability[next] * counted_deg_probs[deg]);
        }
    }
    std::cout << std::fixed;
    cout << setprecision(11) << probability[0] << '\n';
}
