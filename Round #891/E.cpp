...
void solve_test_case() {
    ...
    for (int i = 1; i < n; ++i) {
        weights[i - 1] = numbers[i].value - numbers[i - 1].value;
    }
    ...
    auto prefixes = getPrefixSums(w1);
    auto suffixes = getSuffixSums(w2);

    vector<int> ans (n);
    repeat(n) {
        if (i == 0) {
            ans[numbers[i].index] = suffixes[i] + n;
        } else if (i == n-1) {
            ans[numbers[i].index] = prefixes[i - 1] + n;
        } else {
            ans[numbers[i].index] = prefixes[i - 1] + suffixes[i] + n;
        }
