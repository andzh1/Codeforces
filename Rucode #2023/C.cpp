...
void solve_test_case() {
    ...
    repeat(L) {
        ...
        cin >> s;
        cin >> s;
        int minim = stoi(s.substr(1, s.size() - 2));
        cin >> s;
        int maxim = stoi(s.substr(0, s.size() - 1));
        cin >> s;
        string name = (s.substr(1, s.size() - 4));

        mins.push_back(minim);
        maxs.push_back(maxim);
        names.push_back(name);
    }

    get(T)
    vector<bool> ok_maxs(L), ok_mins(L);
    for (int test = 1; test <= T; ++test) {
        for (int i = 0; i < L; ++i) {
            get(x)
            if (x > maxs[i] || x < mins[i]) {
                cout << "Error\n";
                cout << test << ' ' << names[i] << '\n';
                return;
            }
