...
void solve_test_case() {
    ...
    a.push_back(0);
    n += 2;
    int index_of_first_aval_pos = -1;
    int sum = 0;
    for (int i = 0; i < n; ++i ) {
        if ((a[i] >= 0 && i % 2 == 0) || (a[i] <= 0 && i % 2 != 0)) {
            index_of_first_aval_pos = i; 
            for (int j = i; j < n; ++j) {
                if (a[j] >= 0) {
                    sum += a[j];
                }
    ...
    if (index_of_first_aval_pos == 0) {
        cout << sum << '\n';
        return;
    }

    int pref_sum = 0;
    for (int i = 0; i < index_of_first_aval_pos; ++i) {
        if (a[i] >= 0) {
            pref_sum += a[i];
        }
    }
    ...
    int ans = 0;
    int unused = 0;
    for (int i = 0; i < index_of_first_aval_pos; i += 2) {
        ans = max(ans, pref_sum - unused + a[i]);
        if (a[i + 1] >= 0) unused += a[i+1];
    }
    unused = 0;

    for (int i = 1; i < index_of_first_aval_pos; i += 2) {
        ans = max(ans, pref_sum - unused - a[i]);
        if (a[i] >= 0) unused += a[i];
    }
