...
int getResponse(int pos) {
    cout << "? " << pos << endl;
    get(x)
    return x;
}

int getResponse2(int pos, int k) {
    for (int i = 0; i < k; ++i)
    cout << "? " << pos << endl;
    get(x)
    return x;
...
void solve_test_case() {
    ...
    int tail_len = n - k * simple;
    int tail_xor = 0;

    for (int begin = 2 + (simple - 1) * k; begin <= n - k + 1; ++begin) {
        tail_xor ^= getResponse(begin); 
    }

    cout << "! " << (ans ^ tail_xor) << endl;
